...
import { EntityArrayResponseType, DepartementService } from '../service/departement.service';
...
export class DepartementComponent implements OnInit {
  ...
  trackId = (_index: number, item: IDepartement): number => this.departementService.getDepartementIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.departements = this.refineData(dataFromBody);
  }

  protected refineData(data: IDepartement[]): IDepartement[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IDepartement[] | null): IDepartement[] {
    return data ?? [];
  }
