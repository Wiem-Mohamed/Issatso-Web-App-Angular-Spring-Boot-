...
import { EntityArrayResponseType, PartenaireService } from '../service/partenaire.service';
...
export class PartenaireComponent implements OnInit {
  ...
  trackId = (_index: number, item: IPartenaire): number => this.partenaireService.getPartenaireIdentifier(item);
  ...
  protected refineData(data: IPartenaire[]): IPartenaire[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IPartenaire[] | null): IPartenaire[] {
    return data ?? [];
  }
